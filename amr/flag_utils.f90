...
subroutine init_flag(ilevel)
  ...
  nflag=0
!$omp parallel do private(iskip) schedule(static)
  do ind=1,twotondim
  ...
  if(balance)then
!$omp parallel do private(iskip) reduction(+:nflag) schedule(static)
     do ind=1,twotondim
        iskip=ncoarse+(ind-1)*ngridmax
        do i=1,active(ilevel)%ngrid
...
subroutine test_flag(ilevel)
  ...
  ! Loop over cells
!$omp parallel do private(iskip,ind_grid_son,ok,ind_son,iskip_son,ind_cell_son) reduction(+:nflag) schedule(static)
  do ind=1,twotondim
     iskip=ncoarse+(ind-1)*ngridmax
...
!$omp parallel do private(ngrid,ind_grid,ind_cell,ok,ind,nbors_father_cells,nbors_father_grids,iskip) schedule(static)
...
!$omp parallel do private(ngrid,ind_grid,iskip,ind_cell,ok,nok,xx) reduction(+:nflag) schedule(static)
  do igrid=1,ncache,nvector
     ...
     do ind=1,twotondim
        ...
        !Ensure refinement only happens if level is permitted.
...
!$omp parallel do private(ngrid,ind_grid,ind,iskip,ind_cell) schedule(static)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        do i=1,ngrid
           ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!$omp parallel do private(ngrid,ind_grid,ind,igridn) schedule(static)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        do i=1,ngrid
           ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
...
!$omp parallel do private(ngrid,ind_grid,ind,iskip,ind_cell) reduction(+:nflag) schedule(static)
     do igrid=1,ncache,nvector
        ngrid=MIN(nvector,ncache-igrid+1)
        do i=1,ngrid
           ind_grid(i)=active(ilevel)%igrid(igrid+i-1)
