  ...
  do ind=1,twotondim
     ...
     if(metal.eq.0)then
        ...
        do i=1,nleaf
           ...
           Zsolar(i)=(1.0d0 - (uold(ind_leaf(i),imetal) + uold(ind_leaf(i),imetal+1))/nH(i))/0.02d0 !--- Z = 1 - (H+He)
        end do
     else if(metal.eq.2)then
        do i=1,nleaf
           uold(ind_leaf(i),imetal)=max(uold(ind_leaf(i),imetal),0.0)
           Zsolar(i)=uold(ind_leaf(i),imetal)/nH(i)/0.02d0 !--- Z
        end do
     endif
     ...
     if(use_grackle==1)then
        ...
        do i = 1, nleaf
           ...
           if(metal.eq.0)then
              ...
              gr_metal_density(i) = gr_density(i) - (uold(ind_leaf(i),imetal) + uold(ind_leaf(i),imetal+1))
           else if(metal.eq.2)then
