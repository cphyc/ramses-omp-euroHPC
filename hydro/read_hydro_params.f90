subroutine read_hydro_params(nml_ok)
  ...
  namelist/sf_params/m_star,n_star,T2_star,g_star,del_star &
       ...
       & ,sf_imf,sf_compressive,mstarparticle,mstar_frac,temp_star,SFdiagnostics
...
! MHD sets enums for riemann solvers, hydro just does a string compare
  ...
  !--------------------------------------------------
#ifdef SOLVERmhd
  if(metal.ne.0.and.nvar<(ndim+6))then
#else
  if(metal.ne.0.and.nvar<(ndim+3))then
#endif
  ...
  if(metal.eq.0)then
     ...
     nmetals=2 ! First two are H + He
     do imet=1,size(metal_list)
        if(metal_list(imet)>0)then
           nmetals=nmetals+1
        endif
     enddo
     ! Shift metals and place H and He at the first two indicies of metal_list
     do imet = 0, nmetals-3
        metal_list(nmetals-imet) = metal_list(nmetals-imet-2)
     enddo
     metal_list(1) = 1
     metal_list(2) = 2
     ...
     enddo
     ! Shift metals and place Z at the first index of metal_list
     do imet = 0, nmetals-2
        metal_list(nmetals-imet) = metal_list(nmetals-imet-1)
     enddo
     metal_list(1) = 0
     idelay=imetal+nmetals
  endif
...
!  endif
  ixion=ivirial2
!  if(sf_virial)ixion=ivirial2+1  !Oscar: no extra variable necessary - sigma2 is not to be stored
  ichem=ixion
  ...
  if(myid==1.and.hydro.and.(nvar>ndim+2)) then
     ...
     write(*,*) '   inener   = ',inener
#endif
     if(metal.ne.0)           write(*,*) '   imetal   = ',imetal
     if(delayed_cooling) write(*,*) '   idelay   = ',idelay
     if(sf_virial)then
        write(*,*) '   ivirial1 = ',ivirial1,' Oscar: NB! not used, not allocated'
